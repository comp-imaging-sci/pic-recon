...
static __global__ void FusedBiasActKernel(const FusedBiasActKernelParams<T> p)
{
    ...
    for (int loopIdx = 0; loopIdx < p.loopX && xi < p.sizeX; loopIdx++, xi += blockDim.x)
    {
        ...
        if (p.gain != 0.0f & p.act != 9 & p.act != 10)
            ref /= p.gain;
        ...
        switch (p.act * 10 + p.grad)
        {
            ...
            // leaky softplus
            case 90: y = (x > expRange) ? x : p.alpha*x + (1.0f-p.alpha)*logf(expf(x*beta) + 1.0f)/beta; break;
            case 91: {
                float c = expf(beta * ref);
                float d = c + 1.0f;
                y = x * ( p.alpha + (1.0f-p.alpha) * c / d );
            } break;
            case 92: {
                float c = expf(beta * ref);
                float d = 1.0f / (1.0f + c);
                y = x * (1.0f-p.alpha) * beta * d * (1.0f - d);
            } break;

            // swish
            case 100: y = (x < -expRange) ? 0.0f : x / (expf(-x) + 1.0f); break;
            case 101: { float c = expf(ref); float d = c + 1.0f; y = (ref > halfExpRange) ? x : x * c * (ref + d) / (d * d); } break;
            case 102: { float c = expf(ref); float d = c + 1.0f; y = (ref > halfExpRange) ? 0.0f : x * c * (ref * (2.0f - d) + 2.0f * d) / (d * d * d); } break;
        }
