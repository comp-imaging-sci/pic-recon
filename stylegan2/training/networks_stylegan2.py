...
def modulated_conv2d_layer(x, y, layer_idx, fmaps, kernel, 
    up              =   False, 
    down            =   False, 
    demodulate      =   True, 
    ...
    # zero sections of the dlatent variable except the relevant one
    if   zero_section == 1:
        y = y[ :, int(layer_idx*N/M) : int((layer_idx+1)*N/M) ]
    elif zero_section == 2:
        layer_jdx = layer_idx // 2
        y = y[ :, 2*int(layer_jdx*N/M) : 2*int((layer_jdx+1)*N/M) ]
    elif isinstance(zero_section, list):
        layer_jdx = layer_idx // 2
        y = y[ :, int(N*zero_section[layer_jdx]) : int(N*zero_section[layer_jdx+1])]
...
def G_main(
    ...
    mix_all                 = 0,                        # Do style mixing on all the styles?
    ...
    **kwargs):                                          # Arguments for sub-networks (mapping and synthesis).
    ...
    if (style_mixing_prob is not None) and (not mix_all):
        ...
        with tf.variable_scope('StyleMix'):
            cutoff = np.random.randint(num_layers)
            latents_mix = [tf.random.normal(tf.shape(latents_in))]*cutoff + [tf.random.normal(tf.shape(latents_in))]*(num_layers - cutoff - 1)
            dlatents_mix = [
                components.mapping.get_output_for(l, labels_in, is_training=is_training, **kwargs) \
                    for l in latents_mix]
            
            def mix_latents_func():
                to_be_mixed = [ dlatents[:,:1] ] + [dlt[:,:1] for dlt in dlatents_mix]
                return tf.concat(to_be_mixed, axis=1)
    ...
    elif (style_mixing_prob is not None) and mix_all == 1:

        with tf.variable_scope('StyleMix'):
            latents_mix = [tf.random.normal(tf.shape(latents_in)) for i in range(1, num_layers)]
            dlatents_mix = [
                components.mapping.get_output_for(l, labels_in, is_training=is_training, **kwargs) \
                    for l in latents_mix]
    ...
    elif (style_mixing_prob is not None) and mix_all == 2:

        with tf.variable_scope('StyleMix'):
            latents_mix = [tf.random.normal(tf.shape(latents_in)) for i in range(1, num_layers//2)]
            dlatents_mix = [
                components.mapping.get_output_for(l, labels_in, is_training=is_training, **kwargs) \
                    for l in latents_mix]
            
            def mix_latents_func():
                to_be_mixed = [ dlatents[:,:2] ] + [dlt[:,:2] for dlt in dlatents_mix]
                return tf.concat(to_be_mixed, axis=1)
...
def G_synthesis_stylegan_revised(
    ...
    zero_section        = False,        # Zero all except a section of dlatents 
    ...
    **_kwargs):                         # Ignore unrecognized keyword args.
    ...
    def layer(x, layer_idx, fmaps, kernel, up=False):
        x = modulated_conv2d_layer(x, dlatents_in, layer_idx=layer_idx, fmaps=fmaps, kernel=kernel, up=up, resample_kernel=resample_kernel, fused_modconv=fused_modconv, zero_section=zero_section)
        if randomize_noise:
    ...
    def torgb(res, x): # res = 2..resolution_log2
        with tf.variable_scope('ToRGB_lod%d' % (resolution_log2 - res)):
            return apply_bias_act(modulated_conv2d_layer(x, dlatents_in, layer_idx=res*2-3, fmaps=num_channels, kernel=1, demodulate=False, fused_modconv=fused_modconv, zero_section=zero_section))
...
def G_synthesis_stylegan2(
    ...
    fmap_min            = 1,            # Minimum number of feature maps in any layer.
    fmap_max            = 512,          # Maximum number of feature maps in any layer.
    if_latent_noise     = True,         # Whether or not latent noise is there. CAUTION: Usually do not set to False
    randomize_noise     = True,         # True = randomize noise inputs every time (non-deterministic), False = read noise inputs from variables.
    ...
    resample_kernel     = [1,3,3,1],    # Low-pass filter to apply when resampling activations. None = no filtering.
    fused_modconv       = True,         # Implement modulated_conv2d_layer() as a single fused op?
    zero_section        = 0,            # Zero all but a section of dlatents based on the layer
    **_kwargs):                         # Ignore unrecognized keyword args.
    ...
    for layer_idx in range(num_layers - 1):
        res = (layer_idx + 5) // 2
        shape = [1, 1, 2**res, 2**res]
        noise_inputs.append(int(if_latent_noise)*tf.get_variable('noise%d' % layer_idx, shape=shape, initializer=tf.initializers.random_normal(), trainable=False))
    ...
    def torgb(x, y, res): # res = 2..resolution_log2
        with tf.variable_scope('ToRGB'):
            t = apply_bias_act(modulated_conv2d_layer(x, dlatents_in, layer_idx=res*2-3, fmaps=num_channels, kernel=1, demodulate=False, fused_modconv=fused_modconv, zero_section=zero_section))
