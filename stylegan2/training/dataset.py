...
class TFRecordDataset:
    ...
    # @staticmethod
    def parse_tfrecord_tf(self, record, dtype='float32'):
        features = tf.parse_single_example(record, features={
            'shape': tf.FixedLenFeature([3], tf.int64),
            'data': tf.FixedLenFeature([], tf.string)})
        data = tf.decode_raw(features['data'], getattr(tf, self.dtype))
        return tf.reshape(data, features['shape'])

    # Parse individual image from a tfrecords file into NumPy array.
    # @staticmethod
    def parse_tfrecord_np(self, record, dtype='float32'):
        ex = tf.train.Example()
        ...
        return np.fromstring(data, getattr(np, self.dtype)).reshape(shape)
