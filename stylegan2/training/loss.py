...
def G_logistic_ns_pathreg(G, D, opt, training_set, minibatch_size, pl_minibatch_shrink=2, pl_decay=0.01, pl_weight=2.0, reg_noise_strength_weight=0.0):
    _ = opt
    latents = tf.random_normal([minibatch_size] + G.input_shapes[0][1:])
    labels = training_set.get_random_labels_tf(minibatch_size)
    ...
    # Add regularization on noise scaling
    dim = G.output_shape[2]
    with tf.variable_scope("", reuse=True):
        var = tf.get_variable('G_synthesis/4x4/Conv/noise_strength')
        reg_noise_strength = tf.reduce_sum(var**2)

        sh = 8
        while (sh <= dim):
            var = tf.get_variable(f'G_synthesis/{sh}x{sh}/Conv0_up/noise_strength')
            reg_noise_strength += tf.reduce_sum(var**2)
            var = tf.get_variable(f'G_synthesis/{sh}x{sh}/Conv1/noise_strength')
            reg_noise_strength += tf.reduce_sum(var**2)
            sh *= 2

        reg += reg_noise_strength * reg_noise_strength_weight
    return loss, reg
